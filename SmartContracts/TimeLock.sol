...
contract TimeLock  {
    ...
    constructor(address ebnb_) public {
        ebnb = IBEP20(ebnb_);
    }
   
    //function trans(address sender,uint amt) public returns (bool){
      //  bool result = ebnb.transfer(sender,amt);
    //}
    function deposit(address beneficiary, uint amount, uint releaseTime) public returns(bool success) {
        require(block.timestamp <= releaseTime);
        require(ebnb.transferFrom(msg.sender, address(this), amount));
    ...
    function withdraw(uint lockBoxNumber) public returns(bool success) {
        ...
        require(ebnb.transfer(msg.sender, amount));
        return true;
    } 
